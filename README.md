...
| 스케일 | 대상 문서 처리 | 101건 |
| 스케일 | 평가셋 규모 | 80문항 |
| 검색 품질 | Hit Rate@5 | 재검증 중 |
| 검색 품질 | MRR | 0.988 |
| 검색 품질 | nDCG@5 | 재검증 중 |
| 생성 품질 | Faithfulness | 재검증 중 |
| 생성 품질 | Answer Relevance | 재검증 중 |
| 생성 품질 | Context Precision | 재검증 중 |
| 생성 품질 | Context Recall | 재검증 중 |
| 운영 | Latency (avg) | 5.3초 (`Top-K=5`) |
| 운영 | Cost per Query | `$0.001` (`Top-K=5`) |
